...
- `src/state.rs` — State persistence via bincode to `/host/.zellij-attention-state.bin`
- Build target: `wasm32-wasip1` (Zellij WASM plugin)

## Key Design Decisions

- **One plugin instance per tab** via `default_tab_template` layout pane
...
- `load_plugins` in config.kdl does NOT pass configuration to plugins — use layout instead
